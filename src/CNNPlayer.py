...
class QNetwork:
    ...
    def build_graph(self, name):
        ...
        with tf.variable_scope(name):
            self.input_positions = tf.placeholder(tf.float32, shape=(None, board_width, 3, board_height), name='inputs')
...
class CNNPlayer():
    ...
    def __init__(self, name, reward_discount=0.99, win_value=10.0, draw_value=0.0,
                 loss_value=-10.0, color=RED, learning_rate=0.01, training=True,
    ...
    def get_probs(self, input_pos: [np.ndarray], network: QNetwork) -> ([float], [float]):
        ...
        returns a list of tuples of probabilities and q values of all actions (including illegal ones).
        """

        with self.target_net.graph.as_default():
    ...
    def make_move(self, board: Board) -> (GameResult, bool):
        ...
        _, res, finished = board.move(move, self.color)
        return res, finished
    ...
    def final_result(self, result: GameResult):
        ...
        if self.training and (self.game_counter > self.pre_training_games):
            ...
            # We extract the resulting state, run it through the target network and
